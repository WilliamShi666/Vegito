...
function bandScaleFor(domain: string): BandScale {
  const explicit = domain.match(/\b([0-9])\s*-\s*([0-9])\s*(?:band|score|scoring|point|scale)?/i);
  if (explicit !== null) {
    const min = Number(explicit[1]);
    const max = Number(explicit[2]);
    if (Number.isFinite(min) && Number.isFinite(max) && min < max) return { min, max, label: `${min}-${max}` };
  }
  return { min: 0, max: 9, label: '0-9' };
}
...
export const tutorTeam: Archetype = (params): ForgeSpec => {
  ...
  const bandScale = bandScaleFor(domain);
  ...
  const bandValidator = [
    '#!/usr/bin/env node',
    `// Hard check: the examiner output must carry a numeric band ${bandScale.min}..${bandScale.max} for each criterion.`,
    ...
    `const bad = bands.find((b) => b < ${bandScale.min} || b > ${bandScale.max});`,
  ...
  return {
    ...
    rubrics: [
      {
        name: 'band-score',
        prompt: `Assess the response against the ${domain} band descriptors. For each criterion give a band ${bandScale.label} and a one-line reason.`,
